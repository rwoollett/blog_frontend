import { Mutation, Query } from '../../../generated/graphql-comments';
...
it('ensures that a comment be created on an article slug', async () => {
  ...
  const createComment:Mutation = await ctx.client.request(`
    mutation {
...
it('list all commments for an article', async () => {
  ...
  const articleCommentsResult: Query = await ctx.client.request(`     
